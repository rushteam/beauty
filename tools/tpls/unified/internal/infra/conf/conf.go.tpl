...
type Server struct {
	...
	{{if .EnableWeb}}HTTP HTTP   `mapstructure:"http" yaml:"http"`
	{{end}}{{if .EnableGrpc}}GRPC GRPC   `mapstructure:"grpc" yaml:"grpc"`
	{{end}}Log  Log    `mapstructure:"log" yaml:"log"`
}

{{if .EnableWeb}}// HTTP HTTP服务器配置
type HTTP struct {
