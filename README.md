...
- **实时服务原语**: 52 个可独立组合的实时组件（会话/在场/路由/匹配/排名/调度/钱包/审计/通知/事务/Saga/负载均衡/幂等/延迟队列/状态机/ID 生成/直播 PK/计数聚合/寻路/细粒度锁/连击热度/空间索引/退避重试/事件总线/地理编码/抽卡/冷却/环形缓冲/位图/任务成就/等级/小红点等），纯标准库实现；其中 counter/cooldown/idempotency 可通过 `WithStore` 接 `pkg/kvstore`(Redis 等)实现**跨实例状态共享**，从单机原语升级为多实例生产可用。详见下文 [Realtime Components](#realtime-components)
...
| [docs/realtime-components.md](docs/realtime-components.md) | 实时服务原语总览（52 个组件速查 + 组合范式 + 互补关系表） |
