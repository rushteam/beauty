...
func TestEvent_DefaultTrigger(t *testing.T) {
	...
	var n int
	for ev := range r.Run(context.Background(), llm.Request{Messages: []llm.Message{{Role: llm.User, Content: "q"}}}) {
		n++
...
func TestEvent_WithTrigger(t *testing.T) {
	...
	sawFinal := false
	for ev := range r.Run(ctx, llm.Request{Messages: []llm.Message{{Role: llm.User, Content: "q"}}}) {
