...
- **实时服务原语**: 53 个可独立组合的实时组件（会话/在场/路由/匹配/排名/调度/钱包/审计/通知/事务/Saga/负载均衡/幂等/延迟队列/状态机/ID 生成/直播 PK/计数聚合/寻路/细粒度锁/连击热度/空间索引/退避重试/事件总线/地理编码/抽卡/冷却/环形缓冲/位图/任务成就/等级/小红点等），纯标准库实现；其中 counter/cooldown/idempotency 可通过 `WithStore` 接 `pkg/kvstore`(Redis 等)实现**跨实例状态共享**，从单机原语升级为多实例生产可用。详见下文 [Realtime Components](#realtime-components)
...
#### 多实例部署:只在 leader 上跑(避免重复执行)

多实例部署下,默认每个实例都会各跑一遍定时任务(重复发奖/重复扣款/重复生成报表)。
用 `cron.WithLeaderElector` 接入分布式选主,只有当选 leader 的实例会运行任务:

```go
import (
    clientv3 "go.etcd.io/etcd/client/v3"
    "github.com/rushteam/beauty/pkg/infra/etcd"
    "github.com/rushteam/beauty/pkg/service/cron"
)

client, _ := clientv3.New(clientv3.Config{Endpoints: []string{"127.0.0.1:2379"}})
elector := etcd.NewDLock(client) // 基于 etcd 官方 concurrency 包(Session+Election)

app := beauty.New(
    beauty.WithCrontab(
        cron.WithCronHandler("@every 1m", myJob),
        cron.WithLeaderElector(elector, "myservice-cron"), // 同一组实例用同一个 key
    ),
)
```

失去 leader 身份(进程崩溃/网络分区)时任务立即停止,其余实例自动接管——由 etcd
session 租约保证最终只有一个实例在跑。不配置此项时行为不变。单实例部署/本地开发
可用 `pkg/dlock.NewMemory()` 模拟选举,见 `examples/cron-leader`。

**部署在 k8s 里,不想额外运维 etcd?** 用集群自带的选主能力(同一套
`coordination.k8s.io/v1` Lease 机制,kube-scheduler 等控制面组件也在用):
...
elector := beautyk8s.NewElector(client.CoordinationV1(), beautyk8s.WithNamespace("default"))
...
`pkg/infra/k8s.Elector` 只实现选主(不实现 `Locker`——k8s 的 leaderelection 语义
本就是"持续参选",没有通用一次性互斥锁原语)。
...
| `pkg/idempotency` | 幂等执行（去重 + singleflight 并发合并 + TTL）· 可接 Store 跨实例 | 防重复扣款/发奖 / 请求去重 / 缓存击穿保护 |
| `pkg/kvstore` | 带 TTL 的原子 KV 存储接口 + 内存实现 | counter/cooldown/idempotency 的跨实例后端(Redis 实现自备) |
| `pkg/dlock` | 跨进程分布式锁 + 选主接口 + 内存实现 | 多实例互斥执行 / Cron 只在 leader 跑(后端见 `pkg/infra/etcd.DLock` / `pkg/infra/k8s.Elector`) |
...
| `pkg/counter` | 滑动窗口计数 / 时间窗配额 · 可接 Store 跨实例 | 每日抽卡上限 / 分钟弹幕限频 / 防刷 |
...
| `pkg/loot` | 加权随机抽取（Alias O(1) + 保底 + 不放回） | 抽卡 / 开宝箱 / 掉落 / 直播抽奖 |
| `pkg/cooldown` | 按 key 的冷却（Ready/Remaining/TryTrigger） · 可接 Store 跨实例 | 技能 CD / 每日领取 / 发言间隔 / 防连点 |
| `pkg/ringbuffer` | 定长环形缓冲（覆盖最旧 + O(1) 追加） | 最近 N 条弹幕 / 战绩 / 滚动日志 |
| `pkg/bitmap` | 位图（Set/Count/And/Or + 连续签到） | 千万级签到 / 精确去重 / 权限位 |
| `pkg/questlog` | 任务/成就进度（阈值达成+依赖+领取+重置） | 每日任务 / 成就 / 通行证 / 新手引导 |
| `pkg/leveling` | 经验/等级曲线（线性/多项式/查表） | 角色升级 / 主播等级 / VIP / 亲密度 |
| `pkg/reddot` | 小红点未读聚合树（父节点自动汇总） | App 红点 / 未读汇总 / 分类提示 |
...
| [docs/realtime-components.md](docs/realtime-components.md) | 实时服务原语总览（53 个组件速查 + 组合范式 + 互补关系表） |
