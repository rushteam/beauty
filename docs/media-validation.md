...
> 目的:媒体链路的单测只验证了**产出结构**(如 gohlslib 自身能解析、播放列表含 `#EXTM3U`),
> **没有验证真实播放器能不能放**。上生产前,必须用真实编码流在真播放器里端到端跑一遍。
> 本清单给出可照抄的命令与检查点。
...
- [ ] **动态进出**:第 3 人中途加入,已在房间的两人能听到新人(验证服务端重协商生效);
      有人离开,其音轨从其他人处消失。
...
> 提醒:CDN 回源、DRM、播放器 UI 都在框架之外;本清单只验证框架产出的媒体流本身可被真实播放器消费。
